...
// Buffers are used to handle binary data in Node.js. They are especially useful when dealing with streams of
// data from files, networks, or other I/O sources. Buffers are part of the global scope in Node.js and do not
...
//? 1. Creating Buffers
// You can create buffers in several ways, including allocating a buffer of a specific size, creating a buffer
// from an existing array or string, and from an existing buffer.
{
...
// You can write data to a buffer using the write method. The method allows specifying the encoding, the offset
// to start writing, and the length of data to write.
{
...
//? 6. Slicing Buffers
// The slice method returns a new buffer that references the same memory as the original buffer but
// with a subset of its data.
{
  const buf = Buffer.from("Hello, World!");
...
// Buffers support various encodings for reading and writing string data, such as
// utf8, ascii, hex, and base64.
{
...
// Buffers are optimized for performance in Node.js and are useful for handling large amounts of binary data.
// However, care must be taken to avoid buffer overflow and ensure proper memory management.
{
